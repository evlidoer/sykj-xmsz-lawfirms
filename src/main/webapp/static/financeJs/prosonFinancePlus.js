function saveData(){
	...
	$.ajax({
		type:"post",
		url:"statistics/prosonFinance/upLsjl",
		async:true,
		data : "id=" + $("#properIdPlus").val() + "&startTime=" + $("#startTime").val() + "&endTime=" +
			$("#endTime").val() + "&sumS=" + (parseFloat($("#revenueSpan").html()) + parseFloat($("#commissionSpan").html())) +
			"&sumZ=" + (parseFloat($("#reservedSpan").html()) + parseFloat($("#spendingSpan").html())) + "&sumT=" + $("#sumSpan").html(),
		success : function(msg){
			if(msg.status == "yes") alert("添加成功");
			if(msg.status == "no") alert("添加失败,请重试");
		}
	});
}

$("#lsTable").bootstrapTable({
	url : "statistics/prosonFinance/lstj",
	method : 'get',
	dataType : "json",
